...
namespace settings {
BoolProperty showDebugInfo(true);
BoolProperty showDebugLog(true);
BoolProperty vsyncEnabled(true);
BoolProperty fullScreen;
} // namespace settings

namespace stats {
FloatProperty refreshRate;
UIntProperty FPS;
UIntProperty cpuIdle;
UIntProperty gpuIdle;
} // namespace stats
...
void GLWindow::drawDebugInfo() {
	...
	ImGui::Text("FPS: %i  CPU Idle: %i%%  GPU Idle: %i%%", int(stats::FPS), int(stats::cpuIdle), int(stats::gpuIdle));
