...
int main() {
    ...
    XrSession session{};
    {
        ...
        verify(xrGetInstanceProcAddr(instance, "xrGetOpenGLGraphicsRequirementsKHR", (PFN_xrVoidFunction*)(&xrGetOpenGLGraphicsRequirementsKHR)) == 0);

        XrGraphicsRequirementsOpenGLKHR requirements{};
        verify(xrGetOpenGLGraphicsRequirementsKHR(instance, systemId, &requirements) == 0);
        debug() << "### Requirements max api version:" << XR_VERSION_MAJOR(requirements.maxApiVersionSupported) << "." << XR_VERSION_MINOR(requirements.maxApiVersionSupported);
        debug() << "### Requirements min api version:" << XR_VERSION_MAJOR(requirements.minApiVersionSupported) << "." << XR_VERSION_MINOR(requirements.minApiVersionSupported);
